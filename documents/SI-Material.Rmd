...
title: |
  ...
  | A data-driven semiparametric model of SARS-CoV-2 transmission dynamics in the United States
author: |
  | John M. Drake, Andreas Handel, Éric Marty, Eamon B. O'Dea, Tierney O'Sullivan,
  | Giovanni Righi, and Andrew T. Tredennick
output: 
  pdf_document:
    toc: true
date: "`r Sys.Date()`"
bibliography: COVID-Model.bib
nocite: |
  @ohsfeldt_inpatient_2021, @verity_estimates_2020, @sanche_high_nodate, @mizumoto_estimating_2020, @li_substantial_2020, @zhang_evolving_2020
---
...
The model diagram (Fig. S1) shows the nine-stage SEIR-type compartmental model.
The latent ($L$), infectious ($I$), case ($C$), and hospitalized ($H$) compartments are divided into four sub-compartments to approximate the distribution of durations in each compartment (see Materials and Methods in the main text).
...
We fixed parameters with relatively well-known values to make estimation of other parameters feasible.
The model is too flexible to fit without fixing some parameters.
Table S1 reports the fixed parameters, their values, and the source from which the value was taken.
Table S2 reports the estimated parameters and the parameter value range from which initial parameter sets were drawn (see Materials and Methods in the main text).
...
| Baseline transmission rate of symptomatic individuals | $\beta$ | $\mathcal{R}_0 \times 0.1 \times \frac{1}{N}$ | Assumption |
...
| Relative transmissibility of $C$ to $I_{su/sd}$ | $b_{C}$ | 0.27 | Assumption |
| Relative transmissibility of $H$ to $I_{su/sd}$ | $b_{H}$ | $4.5\times10^{-5}$ | Moghadas et al. (2020) |
| Duration of time in $L$ stages | $1/\gamma_L$ | 4 days | Li et al. (2020) | 
| Duration of time in $I_{a}$ stage | $1/\gamma_{I_{a}}$ | 3.5 days | Li et al. (2020) | 
| Duration of time in $I_{su}$ stages | $1/\gamma_{I_{su}}$ | 6 days | Li et al. (2020) | 
| Duration of time in $I_{sd}$ stage | $1/\gamma_{I_{sd}}$ | $0.5\times1/\gamma_{I_{su}}$ | Assumption  |
| Duration of time in $C$ stages | $1/\gamma_{C}$ | $0.5\times1/\gamma_{I_{su}}$ | Assumption |
| Duration of time in $H$ stages | $1/\gamma_{H}$ | 6 days | Ohsfeldt et al. (2020) |
...
| Rate of increase from $q_\text{min}$ to $q_\text{max}$ | $q_\text{r}$ | 1.1 | Assumption |
| Maximum factor by which diagnosis speed increases | $s_\text{max}$ | 1.0 | Zhang et al. (2020) |
| Day at which diagnosis speed-up factor ($s$) is halfway between 0 and $s_\text{max}$ | $s_\text{half}$ | 30 | Assumption |
| Rate of increase from 0 to $s_\text{max}$ | $s_r$ | 1.1 | Assumption |
| Initial size of susceptible pool | $S(t=1)$ | $N_{\text{state}}$ | Assumption |


\newpage{}

Table: Estimated parameters and starting ranges for MIF estimation procedure. The "expit" function refers to back-transforming the parameter from the logit scale, which was used for estimation.
...
\newpage{}
# Detection probability and diagnosis speed-up functions

The fraction of exposed individuals that are detected and flow into the $I_{sd}$ compartments ($q$) starts at a low level of 0.1 and increases sigmoidally towards a maximum of 0.4, reaching the half way point of 0.25 on the 30th day since the first case notification.
The time to diagnosis ($1/s$) decreases sigmoidally over time from 3 days on day 0 towards a minimum of 1.5 days, reaching the half way point of 2.25 days on the 30th day since the first case notification. 
...
abline(h = mean(q_lim), 
       v = t_halfmax,
       lty="dashed")
axis(1, at = seq(0,340,30))
yticks <- seq(q_min,q_max,length.out = 3)
axis(2, at = yticks, labels = scales::percent(yticks),las=2)
text(x = 75*2, y = mean(q_lim)-diff(q_lim)/4, cex=1, "Detection Probability\nq(t)")
...
text(x = 75*2, y = mean(1/s_lim)-diff(1/s_lim)/4, cex=1, "Diagnosis time\n1/s(t)")
```
