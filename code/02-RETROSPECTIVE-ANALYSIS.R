...
#    1. Calculate derived quantities and trends.
#    2. Calculate mean absolute scaled errors relative to RW model.
#    3. Example plot of cases, deaths, mobility, latent trend, and Re for CA
#    4. Plot of mandates, mobility, and Re for all states
#    5. Correlation analysis of mobility across states and Re across states
#       in two phases of pandemic.
#    6. ANOVA to explain Re as function of mobility and latent trend in
#       two phases of the pandemic.
#    7. Print out max Re range across the 50 states.
...
# max overall -- REPORTED IN MANUSCRIPT
all_states_filters %>%
  dplyr::select(location, R_e) %>%
  group_by(location) %>%
  summarise(val = max(R_e)) %>%
  pull(val) %>%
  range() %>%
  round(., 2)
