loadcleandata <- function(datasource, locations, vars = c("cases","hosps","deaths"), smooth = FALSE, trim = TRUE)
{
  ...
  # # init data frame
  # #################################
  us_dat <- tibble::tibble(
    Location = character(), 
    Date = as.Date(character()),
    Total_Cases = numeric(),
    Total_Deaths = numeric(),
    Total_Hospitalized = numeric(),
    Daily_Cases = numeric(),
    Daily_Deaths = numeric(),
    Daily_Hospitalized = numeric(),
  ...
  if(datasource == "COV"){
    ...
    us_ct_clean <- us_ct_data %>% dplyr::select(c("date","state","positive","negative","total","hospitalized","death")) %>%
      ...
      select(intersect(names(us_dat),names(.)))
    ...
    us_ct_clean$source <- "JHU"
    # combine with data from different sources
    us_dat <- dplyr::bind_rows(us_dat, us_ct_clean)    
  }
  ...
  if(datasource == "NYT"){
    ...
    us_nyt_clean <- us_nyt_data %>% dplyr::select(c(date,state,cases,deaths)) %>%
      ...
      merge(us_popsize %>% select(state=state_full, Population_Size = total_pop)) %>%
      rename(Date = date, Location = state, Total_Deaths = deaths, Total_Cases = cases)  %>%
      select(intersect(names(us_dat),names(.)))
    ...
    us_ct_clean$source <- "NYT"
    # combine with data from different sources
    us_dat <- dplyr::bind_rows(us_dat, us_nyt_clean)    
  }
  ...
  if(datasource == "USF"){
    ...
    usafct_clean$source <- "USF"
    # combine with data from different sources
    us_dat <- dplyr::bind_rows(us_dat, usafct_clean)
  ...
  if(datasource == "JHU"){
    ...
    # combine with data from different sources
    us_dat <- dplyr::bind_rows(us_dat, us_jhu_clean)
  }
  ...
  #################################
    ...
    dplyr::select(date, location, all_of(vars)) %>% 
    group_by(location) %>% 
