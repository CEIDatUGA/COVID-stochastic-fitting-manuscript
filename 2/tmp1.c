...
/* Time: 2022-07-27 18:14:00.493 -0600 */
/* Salt: 602EF31B073F45079827466D */

#include <C:/Program Files/R/R-4.1.1/library/pomp/include/pomp.h>
...
#define min_frac_dead		(__p[__parindex[20]])
#define max_frac_dead		(__p[__parindex[21]])
#define log_half_dead		(__p[__parindex[22]])
...
#define S_0		(__p[__parindex[41]])
#define E1_0		(__p[__parindex[42]])
#define Ia1_0		(__p[__parindex[43]])
#define Isu1_0		(__p[__parindex[44]])
#define Isd1_0		(__p[__parindex[45]])
#define C1_0		(__p[__parindex[46]])
#define H1_0		(__p[__parindex[47]])
#define R_0		(__p[__parindex[48]])
#define D_0		(__p[__parindex[49]])
#define trendO_0		(__p[__parindex[50]])
...
void __pomp_stepfn (double *__x, const double *__p, const int *__stateindex, const int *__parindex, const int *__covindex, const double *__covars, double t, double dt)
{
    ...
    // Ramp-up speed, time at which half-max is reached and max values could be fitted.
    ...
    // function is dmax - (dmax-dmin)*(t/(t+halfdead))
    ...
    // frac_dead = 1/(1+exp(min_frac_dead)) + (1/(1+exp(max_frac_dead)) - 1/(1+exp(min_frac_dead)) ) * (1 - t/(t+exp(log_half_dead)) );
       frac_dead = 1/(1+exp(max_frac_dead)) - (1/(1+exp(max_frac_dead)) - 1/(1+exp(min_frac_dead)) ) * (t/(t+exp(log_half_dead)) );
